...
const ImportExportModal = ({ isOpen, onClose, onImport, students }: ImportExportModalProps) => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      // First line is header row, skip it (lines[0])
      const errors: string[] = []
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (!name || !matric || !program || !className) {
          errors.push(`Row ${i + 1}: Missing name, matric, program, or class`)
          continue
        }
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          {/* Tabs */}
          <div className="mt-4 flex rounded-xl border border-indigo-400/30 bg-slate-950/50 p-1">
            <button
              onClick={() => setActiveTab('import')}
              className={`flex-1 rounded-lg px-4 py-2 text-sm font-semibold transition ${
                activeTab === 'import' ? 'bg-indigo-500 text-white' : 'text-indigo-200 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 rounded-lg px-4 py-2 text-sm font-semibold transition ${
                activeTab === 'export' ? 'bg-indigo-500 text-white' : 'text-indigo-200 hover:text-white'
              }`}
            >
          ...
          {activeTab === 'import' ? (
            <div className="mt-6">
              <div
                ...
                className="cursor-pointer rounded-xl border-2 border-dashed border-indigo-400/30 bg-slate-950/30 p-8 text-center transition hover:border-indigo-400/50 hover:bg-slate-950/50"
              >
                ...
                <p className="mt-1 text-xs text-indigo-200">or drag and drop</p>
              </div>
              <input
                ref={fileInputRef}
                type="file"
              ...
              <button
                ...
                className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-indigo-400/30 px-4 py-3 text-sm font-semibold text-indigo-100 transition hover:bg-white/5"
              >
              ...
              {importResult && (
                <div className="mt-4 rounded-xl border border-indigo-400/20 bg-slate-950/50 p-4">
                  ...
                  {importResult.errors.length > 0 && (
                    <div className="mt-3">
                      <div className="flex items-center gap-2 text-rose-400">
                        ...
                        <span className="text-xs font-semibold">{importResult.errors.length} errors</span>
                      </div>
                      <ul className="mt-2 max-h-24 overflow-y-auto text-xs text-rose-300">
            ...
            <div className="mt-6">
              <div className="rounded-xl border border-indigo-400/20 bg-slate-950/50 p-4">
                <p className="text-sm text-indigo-100">
                  Export all {students.length} students to a CSV file.
                </p>
                <div className="mt-3 flex items-center gap-2 text-xs text-indigo-200">
                  <FileText className="h-4 w-4" />
                  <span>Includes: Name, Matric, Program, Class, Email, Phone, Status</span>
                </div>
              </div>

              <button
                onClick={handleExport}
                className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-indigo-400"
              >
