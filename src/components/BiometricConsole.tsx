...
const BiometricConsole = ({
  ...
  const isExternalCamera =
    cameraConfig.type === "external" && cameraConfig.externalUrl;
  ...
  const handleExternalCameraError = useCallback(() => {
    setCameraError(
      "Failed to connect to external camera. Please check the IP address.",
    );
    setExternalStreamLoading(false);
  }, []);
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            className={`rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 p-1.5 sm:p-2 ${isScanning || isFaceMode || isFingerprintMode ? "animate-pulse" : ""}`}
          >
            {isFaceMode ? (
                  ...
                  <div className="relative">
                    ...
                    >
                      {fingerprintStatus === "success" ? (
                        ...
                        <Fingerprint
                          className={`h-8 w-8 ${fingerprintStatus === "scanning" ? "text-white/70" : "text-white/50"}`}
                        />
                      )}
                    </motion.div>
                  </div>
              ...
              >
                ...
                {scanResult.status === "success" && scanResult.student ? (
                  <div className="mt-2 flex items-center gap-2 rounded-lg bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 p-2">
                    <img
                      src={
                        scanResult.capturedImage || scanResult.student.photoUrl
                      }
