...
const initialState: StoreState = {
  ...
  admins: [
    { id: 'ad-01', name: 'Dr. Karen Li', role: 'System Director', clearance: 'Super' },
  ],
  lecturers: [
    {
      id: 'lec-01',
      name: 'Prof. Solomon Akindele',
      email: 'olasolo@ui.edu.ng',
    ...
    {
      id: 'lec-02',
      name: 'Dr. Nancy Adeyemo',
      email: 'nancy.ade@ui.edu.ng',
      password: 'securepass',
      classId: 'cl-02',
      className: 'Intelligent Systems (CSC 475)',
      status: 'Active',
    ...
    {
      id: 'lec-03',
      name: 'Prof. Ayinla Johnson',
      email: 'ayinla.john@ui.edu.ng',
      password: 'analytics',
      classId: 'cl-03',
      className: 'Human Computer Interaction (CSC 582)',
      status: 'Active',
...
const reducer = (state: StoreState, action: StoreAction): StoreState => {
  switch (action.type) {
    ...
    case 'DELETE_LECTURER':
      return {
        ...state,
        lecturers: state.lecturers.filter((lecturer) => lecturer.id !== action.payload.lecturerId),
        students: state.students.map((student) => ({
          ...student,
          lecturerIds: student.lecturerIds.filter((id) => id !== action.payload.lecturerId),
        })),
      }
    ...
    case 'UPDATE_STUDENT_LAST_SEEN':
      return {
        ...
        students: state.students.map((student) =>
          student.id === action.payload.studentId
            ? { ...student, lastSeen: action.payload.timestamp, status: 'Active' }
            : student,
        ),
      }
    case 'DELETE_LOGS_BY_DATE':
      return {
        ...state,
        logs: state.logs.filter((log) => log.sessionDate !== action.payload.date),
      }
    default:
      return state
  }
